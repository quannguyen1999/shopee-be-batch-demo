...
public class ProductJob {
    ...
    //Import Job
    @Bean
    public Job importFromProduct(JobRepository jobRepository,Step firstStepProduct) {
        return new JobBuilder("productJob", jobRepository)
                .incrementer(new RunIdIncrementer())
                .start(firstStepProduct)
                .build();
    }

    //Define Step
    @Bean
    public Step firstStepProduct(JobRepository jobRepository, DataSourceTransactionManager transactionManager) {
    ...
    @StepScope
    @Bean
    public JdbcCursorItemReader<Product> productItemReader(@Value("#{jobParameters['valueTest']}") String contributionCardCsvFileName) {
        log.info("product");
